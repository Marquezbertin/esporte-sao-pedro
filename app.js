...
function abrirPaginaTime(id) {
    ...
    var cor = esporte ? esporte.cor : "#1a3a6b";
    ...
    var html = '<div class="pagina-time-overlay" onclick="fecharPaginaTime()">' +
        '<div class="pagina-time-wrapper" onclick="event.stopPropagation()">' +
            '<div class="pagina-time-hero" style="background:linear-gradient(135deg,' + cor + '15,' + cor + '05)">' +
                '<button class="pagina-time-voltar" onclick="fecharPaginaTime()">' +
                    '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>' +
                    ' Todos os Times' +
                '</button>' +
                '<div class="pagina-time-hero-inner">' +
                    (time.logo
                        ? '<img src="' + esc(time.logo) + '" alt="' + esc(time.nome) + '" class="pagina-time-logo">'
                        : '<div class="pagina-time-logo pagina-time-logo-placeholder" style="background:' + cor + '20;color:' + cor + '">' + icon + '</div>') +
                    '<div class="pagina-time-hero-texto">' +
                        '<h2 class="pagina-time-nome">' + esc(time.nome) + '</h2>' +
                        '<span class="pagina-time-badge" style="background:' + cor + '20;color:' + cor + '">' + icon + ' ' + (esporte ? esporte.nome : esc(time.esporte)) + '</span>' +
                    '</div>' +
                '</div>' +
            '</div>' +
            '<div class="pagina-time-corpo">' +
                (time.descricao ? '<div class="pagina-time-descricao">' + esc(time.descricao) + '</div>' : '');
    if (hasRedes) {
        html += '<div class="pagina-time-redes">';
        var r = time.redes;
        if (r.instagram) html += '<a href="' + esc(r.instagram) + '" target="_blank" rel="noopener" class="pagina-time-rede" style="background:#fce4ec;color:#c2185b">&#128247; Instagram</a>';
        if (r.facebook) html += '<a href="' + esc(r.facebook) + '" target="_blank" rel="noopener" class="pagina-time-rede" style="background:#e3f2fd;color:#1565c0">&#128250; Facebook</a>';
        if (r.youtube) html += '<a href="' + esc(r.youtube) + '" target="_blank" rel="noopener" class="pagina-time-rede" style="background:#ffebee;color:#c62828">&#9654; YouTube</a>';
        if (r.whatsapp) html += '<a href="' + esc(r.whatsapp) + '" target="_blank" rel="noopener" class="pagina-time-rede" style="background:#e8f5e9;color:#2e7d32">&#128172; WhatsApp</a>';
        if (r.site) html += '<a href="' + esc(r.site) + '" target="_blank" rel="noopener" class="pagina-time-rede" style="background:' + cor + '15;color:' + cor + '">&#127760; Site</a>';
        html += '</div>';
    }
    if (atletas.length > 0) {
        html += '<div class="pagina-time-atletas">' +
            '<h3 class="pagina-time-atletas-titulo">Atletas</h3>' +
            '<p class="pagina-time-atletas-sub">Jogadores vinculados a este time</p>' +
        ...
        atletas.forEach(function (a) {
            html += '<div class="pagina-time-atleta-card" onclick="fecharPaginaTime();setTimeout(function(){abrirLinktree(\'' + a.id + '\',\'atleta\')},50)">' +
                (a.imagem
                    ? '<img src="' + esc(a.imagem) + '" alt="' + esc(a.nome) + '" class="pagina-time-atleta-foto">'
                    : '<div class="pagina-time-atleta-foto pagina-time-atleta-foto-placeholder">&#x1F3C3;</div>') +
...
// ===== POPUP COBERTURA ESPECIAL (removido - evento ja ocorreu) =====
...
function gerarPDFOrcamento(id) {
    ...
    var itensHtml = orcamento.itens.map(function (item, idx) {
        ...
        return "<tr>" +
            '<td style="padding:8px 10px;border-bottom:1px solid #e2e8f0;text-align:center;font-size:12px;">' + (idx + 1) + "</td>" +
            '<td style="padding:8px 10px;border-bottom:1px solid #e2e8f0;font-size:12px;">' + esc(item.nome) + "</td>" +
            '<td style="padding:8px 10px;border-bottom:1px solid #e2e8f0;text-align:center;font-size:12px;">' + item.quantidade + "</td>" +
            '<td style="padding:8px 10px;border-bottom:1px solid #e2e8f0;text-align:right;font-size:12px;">' + unit + "</td>" +
            '<td style="padding:8px 10px;border-bottom:1px solid #e2e8f0;text-align:right;font-size:12px;font-weight:700;">' + sub + "</td>" +
            "</tr>";
    }).join("");
    ...
    var html = '<div id="pdf-orcamento" style="font-family:Arial,Helvetica,sans-serif;width:820px;padding:50px;background:#ffffff;color:#1e293b;box-sizing:border-box;">' +
        '<div style="display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #d4a017;padding-bottom:16px;margin-bottom:20px;">' +
            '<div style="font-size:26px;font-weight:900;color:#0a1628;">ESPORTE <span style="color:#d4a017;">SAO PEDRO</span><div style="font-size:11px;color:#64748b;font-weight:400;margin-top:2px;">Portal Esportivo de Sao Pedro/SP</div></div>' +
            ...
            "</div>" +
        "</div>" +
        '<div style="display:flex;justify-content:space-between;margin-bottom:24px;padding:14px 20px;background:#f8fafc;">' +
            "<div>" +
        ...
        '<table style="width:100%;border-collapse:collapse;margin-bottom:24px;">' +
            "<thead>" +
                '<tr style="background:#0a1628;color:#ffffff;">' +
                    '<th style="padding:10px 12px;text-align:center;font-size:11px;">#</th>' +
                    '<th style="padding:10px 12px;text-align:left;font-size:11px;">Item</th>' +
                    '<th style="padding:10px 12px;text-align:center;font-size:11px;">Qtd</th>' +
                    '<th style="padding:10px 12px;text-align:right;font-size:11px;">Valor Unit.</th>' +
                    '<th style="padding:10px 12px;text-align:right;font-size:11px;">Subtotal</th>' +
                "</tr>" +
            "</thead>" +
            "<tbody>" + itensHtml + "</tbody>" +
        "</table>" +
        '<div style="display:flex;justify-content:flex-end;margin-bottom:24px;">' +
            '<div style="background:#0a1628;color:#ffffff;padding:12px 36px;text-align:center;">' +
                '<div style="font-size:10px;text-transform:uppercase;letter-spacing:1px;opacity:0.7;">Total</div>' +
                '<div style="font-size:26px;font-weight:800;margin-top:2px;">' + total + "</div>" +
            "</div>" +
        "</div>" +
        (obs ? '<div style="padding:14px 20px;background:#fef9c3;margin-bottom:24px;border-left:4px solid #d4a017;">' +
        ...
        "</div>" : "") +
        '<div style="border-top:2px solid #e2e8f0;padding-top:16px;text-align:center;font-size:11px;color:#94a3b8;">' +
    ...
    html2canvas(div, {
        ...
        var pageW = pdf.internal.pageSize.getWidth() - 10;
        ...
        if (finalH <= pageH) {
            pdf.addImage(imgData, "JPEG", 5, 5, pageW, finalH);
        } else {
            var y = 0;
            while (y < finalH) {
                if (y > 0) pdf.addPage();
                pdf.addImage(imgData, "JPEG", 5, 5 - y, pageW, finalH);
                y += pageH;
            }
        }
        pdf.save("orcamento-esporte-sao-pedro-" + orcamento.id.slice(-8) + ".pdf");
        ...
        if (div.parentNode) document.body.removeChild(div);
        hideLoading();
        showToastErro("Erro ao gerar PDF: " + (err.message || "erro desconhecido"));
    });
}
...
function compartilharOrcamento(id) {
    ...
    var total = "R$ " + ((parseFloat(orcamento.total) || 0).toFixed(2).replace(".", ","));
    var cliente = orcamento.cliente || "Sem nome";
    var data = orcamento.data || "";
    var itensResumo = orcamento.itens.map(function (item) {
        return "- " + item.nome + " x" + item.quantidade + " = R$ " + ((item.subtotal || 0).toFixed(2).replace(".", ","));
    }).join("\n");

    var assunto = encodeURIComponent("Orcamento Esporte Sao Pedro - " + cliente);
    var corpo = encodeURIComponent(
        "Ola " + cliente + ",\n\n" +
        "Segue o orcamento do Esporte Sao Pedro:\n\n" +
        itensResumo + "\n\n" +
        "Total: " + total + "\n\n" +
        "Data: " + data + "\n\n" +
        "Atenciosamente,\nEquipe Esporte Sao Pedro\n" +
        "https://marquezbertin.github.io/esporte-sao-pedro/\n\n" +
        "---\nEste e um documento gerado automaticamente."
    );

    if (navigator.share) {
        navigator.share({
            title: "Orcamento - Esporte Sao Pedro",
            text: "Orcamento para " + cliente + " - Total: " + total,
            url: "https://marquezbertin.github.io/esporte-sao-pedro/"
        }).catch(function () {});
    } else {
